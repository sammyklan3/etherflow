import React from 'react';

export default function Networks() {
  const networks = [
    {
      name: 'Ethereum',
      description: 'Main Ethereum network',
      icon: 'data:image/svg+xml;base64,',
      bgColor: 'bg-gray-100',
      textColor: 'text-gray-900'
    },
    {
      name: 'Base',
      description: 'Coinbase L2 network',
      icon: 'data:image/svg+xml;base64,',
      bgColor: 'bg-blue-100',
      textColor: 'text-blue-900'
    },
    {
      name: 'Sepolia',
      description: 'Ethereum test network',
      icon: 'data:image/svg+xml;base64,',
      bgColor: 'bg-purple-100',
      textColor: 'text-purple-900'
    },
    {
      name: 'Base Sepolia',
      description: 'Base test network',
      icon: 'data:image/svg+xml;base64,',
      bgColor: 'bg-indigo-100',
      textColor: 'text-indigo-900'
    }
  ];

  return (
    <div id="networks" className="py-16 bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h2 className="text-3xl font-bold text-gray-900 mb-4">Supported Networks</h2>
          <p className="text-gray-600 max-w-2xl mx-auto">
            Transfer your assets across multiple networks with seamless integration
          </p>
        </div>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {networks.map((network) => (
            <div key={network.name} className={`${network.bgColor} rounded-xl p-6 transition-transform hover:scale-105`}>
              <div className="flex items-start space-x-4">
                <img src={network.icon} alt={network.name} className="w-12 h-12" />
                <div>
                  <h3 className={`font-semibold ${network.textColor} text-lg`}>{network.name}</h3>
                  <p className="text-gray-600 text-sm mt-1">{network.description}</p>
                </div>
              </div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
}